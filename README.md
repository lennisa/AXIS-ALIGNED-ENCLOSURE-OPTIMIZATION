...
In the city of Axion Prime, buildings are placed on a 2D grid, each associated with a weight (cost).
The objective is to enclose at least K buildings inside a simple, closed, axis-aligned polygon (edges parallel to the coordinate axes) such that the total cost is minimized.
...
C is the minimum total cost (printed with at least 6 decimal places)

Each line represents one axis-aligned edge of the polygon
